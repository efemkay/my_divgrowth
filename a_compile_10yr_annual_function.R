...
Compile_10yr_Annual <- function(scode_list, data_loc="./Stock Data/"){
  ...
  for (scode in scode_list) {
    ...
    breakout=FALSE
    weblist = read_html(paste0(data_loc, scode, '.html'))
    sname = (html_node(weblist, "title") %>% html_text() %>% strsplit(" ") %>% unlist())[1]
    rm(list='weblist')
    webtable = readHTMLTable(doc=paste0(data_loc, scode, '.html'))
