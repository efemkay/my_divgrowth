...
zipfiles <- list.files(path = "./Price Data - hist", pattern = "*.zip", full.names = TRUE)

unzip(zipfile = zipfiles[1])
lapply(X = zipfiles, FUN = unzip, exdir = "./Price Data - hist")

pricefiles = list.files(path = "./Price Data - hist", pattern = "history_.*.csv", full.names = TRUE)

lapply(X = pricefiles, FUN = unzip, exdir = "./Price Data - hist")

df_tmp = read.csv(file=pricefiles[1])
